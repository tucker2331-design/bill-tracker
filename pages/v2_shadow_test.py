...
st.set_page_config(page_title="v38 Raw Truth", page_icon="🧬", layout="wide")
st.title("🧬 v38: The 'Raw Truth' (OG Dev Box Restored)")
...
filter_text = st.sidebar.text_input("Filter Raw Table (e.g. 'Convenes')")
...
# --- COMPONENT 1: THE SCRAPER (Returns BOTH Blocks & Raw Lines) ---
@st.cache_data(ttl=300)
def fetch_scraper_data():
    """
    Returns a tuple: (schedule_map, raw_line_data)
    - schedule_map: For the matcher
    - raw_line_data: For the OG Dev Box
    """
    schedule_map = {} 
    raw_line_data = [] # For the Dev Table
    ...
    try:
        ...
        # We grab all relevant tags to preserve structure in the debug table
        ...
        for tag in all_tags:
            ...
            # 1. DATE DETECTION (Broad Regex)
            if any(day in text for day in ["Monday,", "Tuesday,", "Wednesday,", "Thursday,", "Friday,"]):
                # Save raw line info
                raw_line_data.append({"date": "DATE HEADER", "text": text, "tag": tag.name})
...
with st.spinner("Scraping Raw Data..."):
    daily_blocks_map, raw_debug_data = fetch_scraper_data()

# --- THE OG DEV BOX ---
if show_debug:
    st.subheader("🔍 Raw Scraper Output (What the bot sees)")
    st.info("Check this table. Are the missing meetings (e.g. Next Week) actually listed here?")
