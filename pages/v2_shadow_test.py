...
# --- SMART SYNC CLOCK (Frontend Auto-Refresh) ---
components.html(
    """
    <script>
            ...
            if i == 0: 
                ...
                for _, row in final_df.iterrows():
                    ...
                    if happened_today:
                        ...
                        noise_keywords = ["fiscal impact", "statement from", "note filed", "assigned", "referred", "docketed"]
    ...
    if df_bugs.empty:
        ...
        if open_bugs.empty:
            ...
            with st.expander(f"🚨 Unrecognized Status Phrase ({len(vocab)})"):
                st.write("**What it means:** The state clerk used a new action phrase that our system doesn't know how to categorize.")
            ...
            with st.expander(f"🧭 Unmapped Committee Name ({len(routing)})"):
                st.write("**What it means:** A bill was referred to a committee, but the name is spelled drastically wrong or isn't on our master list.")
            ...
            with st.expander(f"🗂️ Missing Topic Keyword ({len(sorting)})"):
                st.write("**What it means:** The engine couldn't identify the bill's subject because none of the words in its title are in our dictionary.")
            ...
            with st.expander(f"🔌 Background Sync Failures ({len(sync)})"):
                st.write("**What it means:** The automated GitHub script failed to run and update the database.")
                st.write("**How to fix:** Log into GitHub, go to the 'Actions' tab, and check for server errors or expired security tokens.")
