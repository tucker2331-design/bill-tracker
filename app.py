import streamlit as stimport pandas as pdimport requestsimport timeimport refrom datetime import datetime, timedeltaimport pytzfrom slack_sdk import WebClientfrom slack_sdk.errors import SlackApiErrorfrom bs4 import BeautifulSoup # <--- REQUIRED: pip install beautifulsoup4# --- CONFIGURATION ---
...
SUBS_URL = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&sheet=Subscribers"# --- VIRGINIA LIS DATA FEEDS ---
...
st.set_page_config(page_title="VA Bill Tracker 2026", layout="wide")# --- EXPANDED SMART CATEGORIZATION ---
...
}# --- HELPER FUNCTIONS ---def determine_lifecycle(status_text, committee_name):
...
return "📣 Out of Committee"def get_smart_subject(title):
...
return "📂 Unassigned / General"def normalize_text(text):
...
return " ".join(text.split())def clean_committee_name(name):
...
return base_name.title()def clean_status_text(text):
...
return text# --- NEW: ROBUST SCRAPER (Fixed for "Minutes After") ---@st.cache_data(ttl=600)def fetch_schedule_from_web():
...
return schedule_map# --- DATA FETCHING (DIRECT FROM LIS) ---@st.cache_data(ttl=300) def fetch_lis_data():
...
return datadef get_bill_data_batch(bill_numbers, lis_df):
...
return pd.DataFrame(results)# --- ALERTS ---def check_and_broadcast(df_bills, df_subscribers, demo_mode):
...
st.sidebar.info("💤 No new updates needed.")# --- UI COMPONENTS ---def render_bill_card(row):
...
st.divider()def render_master_list_item(df):
...
st.markdown(f"🔗 [View Official Bill on LIS]({lis_link})")# --- MAIN APP ---

st.title("🏛️ Virginia General Assembly Tracker")

est = pytz.timezone('US/Eastern')

current_time_est = datetime.now(est).strftime("%I:%M %p EST")if 'last_run' not in st.session_state:

st.session_state['last_run'] = current_time_est# --- SIDEBAR CONTROLS ---

demo_mode = st.sidebar.checkbox("🛠️ Enable Demo Mode", value=False)

col_btn, col_time = st.columns([1, 6])with col_btn:
...
st.rerun()with col_time:

st.markdown(f"**Last Refreshed:** `{st.session_state['last_run']}`")# 1. LOAD USER DATAtry:
...
st.stop()# 2. FETCH LIS DATA

lis_data = fetch_lis_data()

bills_to_track = sheet_df['Bill Number'].unique().tolist()# 2b. FETCH WEB SCHEDULE (HYBRID MODE)

web_schedule_map = fetch_schedule_from_web()if bills_to_track:
...
st.caption("-")# --- DEV DEBUGGER ---with st.sidebar:
