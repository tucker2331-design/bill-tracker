...
print("🚀 Waking up Enterprise Calendar Worker (With Fiefdom Extractor)...")
...
def run_calendar_update():
    ...
    try:
        ...
        if sched_res.status_code == 200:
            ...
            for meeting in schedules:
                ...
                # Strip HTML for the UI description
                clean_desc = re.sub(r'<[^>]+>', '', raw_desc).strip()
                
                # Look for hidden URLs in the raw HTML before we clean it
                agenda_url = None
                link_match = re.search(r'href=[\'"]?([^\'" >]+)', raw_desc)
                if link_match and ("agenda" in raw_desc.lower() or "docket" in raw_desc.lower()):
                ...
                # Ignore floor sessions for docket mapping
                ...
                # If the central system is empty, but an agenda link exists, fire the extractor!
                if not scheduled_bills and agenda_url and not is_cancelled:
                    print(f"🕵️‍♂️ Missing Data Detected! Deploying Extractor to: {agenda_url}")
                    scheduled_bills = extract_rogue_agenda(agenda_url)
                
                # Map the bills to the UI
                if scheduled_bills:
    ...
    if not final_df.empty:
        ...
        print("✅ SUCCESS: Hybrid Extraction Pipeline complete.")
    else:
