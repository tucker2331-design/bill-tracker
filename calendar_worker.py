...
# creation (_get_or_create_legevent_tabs) and growth (_persist's
# _ensure_row_capacity) so BOTH paths respect the same budget (Codex P2,
# PR #61: creation must be budget-aware too, not just growth).
LEGEVENT_WORKBOOK_CELL_CEILING = 9_500_000


def _workbook_cell_headroom(spreadsheet, col_count):
    """Rows of `col_count` width that can still be added under the ceiling.

    Returns an int (>=0) or None if the workbook can't be measured (in which
    case callers fall through to an unguarded attempt — the raw API error is
    still caught downstream). Sums allocated cells across all worksheets,
    which is what Google counts against the 10M cap.
    """
    try:
        ...
        try:
            ...
            try:
                ...
                if _old_rows < rows_init:
                    # Codex P2 (PR #61): clamp the one-step lift to the
                    # workbook cell budget so growing a near-cap workbook
                    # can't trip the 10M cap. _ensure_row_capacity in the
                    # persist path grows further on demand (and alerts) if
                    # the data genuinely needs more than fits here.
                    _headroom = _workbook_cell_headroom(sheet, len(header))
                    _target_rows = (rows_init if _headroom is None
                                    else min(rows_init, _old_rows + _headroom))
                    if _target_rows > _old_rows:
                        ws.add_rows(_target_rows - _old_rows)
                        print(f"📈 Grew {tab_name} to {_target_rows:,} rows (was {_old_rows:,}).")
            except Exception as _grow_err:
                push_alert(
                    f"Failed to grow {tab_name} toward {rows_init:,} rows: {_grow_err}. "
            ...
            try:
                # Codex P2 (PR #61): clamp the initial allocation to the
                # workbook cell budget. Creating a 120k-row tab in a near-cap
                # workbook would fail `add_worksheet` (exceeds 10M) and return
                # None, disabling the LegEvent cache ENTIRELY — worse than the
                # old 25k that would have fit. Instead create a smaller tab
                # (header + whatever fits) and let the guarded persist grow it.
                _headroom = _workbook_cell_headroom(sheet, len(header))
                _create_rows = (rows_init if _headroom is None
                                else max(2, min(rows_init, _headroom)))
                ws = sheet.add_worksheet(title=tab_name, rows=_create_rows, cols=len(header))
                ws.update(values=[header], range_name="A1")
                print(f"📝 Created {tab_name} tab ({_create_rows:,} rows × {len(header)} cols) with header.")
                return ws
            except Exception as e:
                push_alert(
...
def _build_legevent_refresh_queue(
    ...
    for bill in sorted(candidate_bills):  # sort for deterministic order
        ...
        if cached is None or not has_events:
            tier_a.append(bill)
            continue
        # PR-C7.1v: schema-incomplete cache migration (assumptions_audit #75).
        # A bill whose cached events predate the CommitteeName column carry the
        # "?" sentinel; the structural time-recovery REFUSES "?" (it can't tell a
        ...
        if any(_e.get("CommitteeName", "?") == "?"
               for _e in (events_cache.get((bill, session_5d)) or [])):
            tier_b.append(bill)
            schema_backfill += 1
            continue
...
def _persist_legevent_cache(
    ...
    def _ensure_row_capacity(ws, needed_rows, tab_name, dedup_key):
        ...
        if needed_rows <= current:
            return needed_rows
        # MINIMUM growth = just enough to fit the data; DESIRED adds a buffer
        # so we don't resize every cycle.
        min_add = needed_rows - current
        desired_add = min_add + ROW_GROWTH_BUFFER
        add = desired_add
        headroom_rows = _workbook_cell_headroom(ws.spreadsheet, ws.col_count)
        if headroom_rows is not None:
            # Gemini HIGH (PR #61): gate the CRITICAL alert + truncation on the
            # MINIMUM need, not the buffered target. A buffer that would push
            # past the ceiling is NOT data loss — only the data itself failing
            # to fit is. Alerting on the buffer is false operational noise.
            if min_add > headroom_rows:
                # The data genuinely doesn't fit under the cell ceiling.
                writable = current + headroom_rows
                push_alert(
                    f"{tab_name} needs {needed_rows:,} rows but the workbook cell "
                    f"budget only allows {writable:,} (ceiling "
                    f"{LEGEVENT_WORKBOOK_CELL_CEILING:,} cells). Persisting "
                    f"{writable:,}; {max(0, needed_rows - writable):,} rows dropped "
                    f"this cycle — cache routing will be INCOMPLETE. ACTION: prune "
                    f"Schedule_Witness (L3b audit) or trim LegEvent retention.",
                    status="ALERT", category="DATA_ANOMALY", severity="CRITICAL",
                    dedup_key=f"{dedup_key}_cell_budget",
                )
                if headroom_rows > 0:
                    try:
                        ws.add_rows(headroom_rows)
                    except Exception:
                        return current
                return writable
            # Data fits; cap the buffered growth to the available headroom
            # (no alert — nothing dropped, we just grow a little less).
            add = min(desired_add, headroom_rows)
        try:
            ws.add_rows(add)
            print(f"📈 Grew {tab_name} grid {current:,} → {current + add:,} rows "
                  f"(data needs {needed_rows:,}).")
        except Exception as e:
            push_alert(
                f"Failed to grow {tab_name} by {add:,} rows: {e}. Persist "
...
def get_active_session_info(http_session):
    ...
    # auth_failed distinguishes a 401/403 (the WebAPIKey was rotated/revoked — a
    # PERMANENT failure needing a human to update the key) from a transient outage
    # (recovers next cycle). The caller raises a distinct CRITICAL on the former so a
    # key rotation can't hide inside the every-cycle "OFFLINE" noise.
    auth_failed = False
    try:
        res = http_session.get("https://lis.virginia.gov/Session/api/GetSessionListAsync", headers=HEADERS, timeout=10)
        auth_failed = res.status_code in (401, 403)
        if res.status_code == 200:
            raw_json = res.json()
            sessions = raw_json.get('Sessions', []) if isinstance(raw_json, dict) else raw_json
            if not isinstance(sessions, list) or len(sessions) == 0: return None, False, auth_failed
...
def run_calendar_update():
    ...
    _VALID_ORIGINS = {
        ...
        # I3 below.
        "legislation_event",
        # PR-C7.1g (#66 fold-in): journal_default rows the structural router
        # classifies as administrative (route=="admin") whose resolver we
        # deliberately skip (no wrong document-batch time). NOT a concrete
    ...
    if not session_data:
        ...
        if _session_auth_failed:
            # A 401/403 on the gateway = the WebAPIKey was rotated/revoked. This will
            # NOT self-heal — every cycle stays offline-degraded until a human updates
            # API_KEY / LIS_PUBLIC_API_KEY. Distinct CRITICAL so it can't hide inside
            # the transient-outage OFFLINE noise (fallback-liveness, audit #52).
            push_system_alert(
                "🔑 LIS API AUTH FAILURE (HTTP 401/403): the WebAPIKey was likely rotated/revoked. "
                "The worker is running OFFLINE-DEGRADED and will NOT recover until API_KEY / "
                "LIS_PUBLIC_API_KEY are updated in calendar_worker.py. Re-source the public key from "
                "the LIS SPA bundle (handleTitle.js).",
                status="CRITICAL", category="API_FAILURE", severity="CRITICAL",
                dedup_key="lis_api_auth_failure",
            )
        else:
        ...
        try:
            ...
            if sched_res.status_code == 200:
                ...
                for meeting in schedules:
                    ...
                    # widens off-season. Mirrors the final in_window write-gate, which already uses
                    # effective_scrape_end (the two upper bounds must agree or a kept meeting gets re-dropped).
                    if not (test_start_date <= meeting_date <= effective_scrape_end): continue
    ...
    if not df_past.empty:
        ...
        source_miss_counts["legevent_schema_backfill"] = legevent_tiers["schema_backfill"]
        ...
        print(
            ...
            f"schema_backfill={legevent_tiers['schema_backfill']}, "
        ...
        if os.environ.get("STM_REPLAY_SIM") == "1":
            ...
            try:
                ...
                _rp_stride = max(1, int(os.environ.get("STM_REPLAY_STRIDE", "5") or "5"))
                # Pre-extract per-day row ADDITIONS in ONE pass (column zip — no groupby/
                # iterrows on the growing slice every day, Gemini #152). Then walk days
                # accumulating per-bill rows and re-hashing ONLY the bills that changed that
                # day; run the (expensive) full STM + compare every Nth day (stride) so the
                # ~90 STM re-runs — the real cost — drop to ~18.
                _norm = df_past["ParsedDate"].dt.normalize()
                _desc_vals = (df_past[desc_col].astype(str) if desc_col in df_past.columns
                              else [""] * len(df_past))
                _refid_vals = (df_past[refid_col].astype(str) if (refid_col and refid_col in df_past.columns)
                               else [""] * len(df_past))
                _day_adds = {}
                for _bv, _dn, _pdv, _dv, _rv in zip(df_past["CleanBill"].astype(str), _norm,
                                                    df_past["ParsedDate"], _desc_vals, _refid_vals):
                    if pd.isna(_dn):
                        continue
                    _bb = _bv.strip()
                    if not _bb:
                        continue
                    _ds = _pdv.strftime("%Y-%m-%d") if pd.notna(_pdv) else ""
                    _day_adds.setdefault(_dn, []).append((_bb, (_ds, str(_dv or ""), str(_rv or ""))))
                _all_days = sorted(_day_adds)
                print(f"🎬 REPLAY SIM: {len(_all_days)} session-days (stride {_rp_stride}), LOCAL, network-free...")
                _acc_rows, _acc_hash, _rp_cache = {}, {}, {}
                for _di, _day in enumerate(_all_days):
                    _changed = set()
                    for _bb, _rt in _day_adds[_day]:                 # accumulate EVERY day (HISTORY-as-of-D exact)
                        _acc_rows.setdefault(_bb, []).append(_rt)
                        _changed.add(_bb)
                    for _bb in _changed:                             # re-hash ONLY bills that changed today
                        _acc_hash[_bb] = _hash_history_rows_for_bill(_acc_rows[_bb])
                    if _di % _rp_stride != 0 and _di != len(_all_days) - 1:
                        continue                                     # only CHECK (full STM + diff) on stride days
                    _hashes_d = dict(_acc_hash)
                    _df_d = df_past[_norm <= _day]
                    ...
                    for _bb in set(_byb) | set(_hashes_d):
                        _rp_cache[_bb] = {"hash": _hashes_d.get(_bb, ""), "events": _byb.get(_bb, [])}
            ...
            if _rp_days and _rp_mismatch == 0:
                print(f"🎬✅ REPLAY SIM PASSED — {_rp_days} checkpoints across the session, incremental==full at "
                      f"EVERY one. The engine reproduces the 2026 calendar on real in-season deltas. ({_rp_secs:.0f}s)")
            elif _rp_mismatch:
                push_system_alert(
                    f"REPLAY SIM: incremental != full at {_rp_mismatch}/{_rp_days} session checkpoints — a HISTORY-delta "
