...
- [ ] **Step 6 — INCREMENTAL STM (the real IN-SEASON lever; Stage 2 only helps off-season/quiet).** Gemini (2026-06-16) correctly flagged that the short-circuit (Step 4) almost never fires in-season (HISTORY changes constantly), and the active-session bottleneck is the full 60k-row STM re-run on every cycle a new action lands. **Decisive feasibility finding (code-read, 2026-06-16):** the STM has **NO cross-bill state** — the only cross-row state is `bill_locations[bill_num]` + `last_seen_date[bill_num]` (both bill-keyed); `docket_memory`/`convene_times`/schedule map are read-only inputs in the loop. So it's **per-bill decomposable** → reprocess only the bills whose history changed (reuse `_hash_history_rows_for_bill`, already used for LegEvent hydration), carry forward the rest.
  ...
  - **STATUS 2026-06-17: the flip is BUILT (flag-gated, shadow-validated) — PR pending; default off, no production change on merge.** Validation: order-invariance ✅, cache shadow ✅ (3645 reused / 0 recomputed / match), day-by-day replay ✅ (28/28). `STM_INCREMENTAL_PRIMARY` env: `""`=full STM (default), `shadow`=full drives Sheet1 + incremental runs in isolation and proves events **and** breaker telemetry identical, `1`=incremental drives Sheet1. Reuse gated on cache-loaded AND the shared-input signature unchanged (`_compute_stm_shared_sig`, now ONE shared definition).
    ...
    - **ROLLOUT (do NOT skip):** set repo var `STM_INCREMENTAL_PRIMARY=shadow`, confirm `✅ INCREMENTAL-PRIMARY SHADOW MATCH` (calendar + breaker) over a window of real cycles **including crossover week** + capture the logged non-breaker telemetry deltas, build the event-derived reproduction, THEN set `=1`. CRITICAL alert (`dedup_key=stm_incremental_primary_divergence`) on any calendar/breaker divergence.
