...
- [ ] **Profile the ~16-19 min worker run** (it's why the cron is 3h, not 15m). Step 1: instrument each phase with timing prints (HISTORY fetch / LegEvent hydration / row processing / **Sheet1 write** / cache+witness persistence) and run once to get the real breakdown — don't guess. **Code-level hypothesis (the likely heavies):** (a) the **full Sheet1 clear()+rewrite** every cycle — ~37.5k rows × 14 cols ≈ 525k cells, chunked at 5k, network-bound; (b) the **cache/witness persistence** writes to several tabs; (c) per-row CPU processing over 37k+ rows. The LIS hydration is already incremental (per-bill HISTORY hash, `_hash_history_rows_for_bill`) so it's NOT the steady-state cost. **Biggest likely win:** make the Sheet1 write **incremental** — diff new final_df vs the live sheet, batch-update only CHANGED rows instead of clear()+full-rewrite (most steady-state cycles change few rows). Risk: clear()+rewrite is idempotent/safe; an incremental diff is more complex and must be correctness-tested. **Then:** a session-aware cadence (15-min in-session via the schedule + a cheap-exit when HISTORY hasn't mutated; 3h off-season). Pairs with the chained sentinel (#SRE) so detection is immediate on each cycle regardless of cadence.
