...
- [ ] **Step 6 — INCREMENTAL STM (the real IN-SEASON lever; Stage 2 only helps off-season/quiet).** Gemini (2026-06-16) correctly flagged that the short-circuit (Step 4) almost never fires in-season (HISTORY changes constantly), and the active-session bottleneck is the full 60k-row STM re-run on every cycle a new action lands. **Decisive feasibility finding (code-read, 2026-06-16):** the STM has **NO cross-bill state** — the only cross-row state is `bill_locations[bill_num]` + `last_seen_date[bill_num]` (both bill-keyed); `docket_memory`/`convene_times`/schedule map are read-only inputs in the loop. So it's **per-bill decomposable** → reprocess only the bills whose history changed (reuse `_hash_history_rows_for_bill`, already used for LegEvent hydration), carry forward the rest.
  ...
  - **STATUS 2026-06-17: validation DONE, the PRIMARY flip is the remaining work — and carries a telemetry risk.** Order-invariance ✅, real-cycle shadow ✅ (3645 reused / 0 recomputed / match), day-by-day replay ✅ (28/28 checkpoints). Mechanically tractable: every event = exactly the 14 `_STM_EVENT_KEY_FIELDS`, so `STM_Bill_Cache` keys reconstruct full events losslessly (`dict(zip(_STM_EVENT_KEY_FIELDS, key))`) — no cache-format change. **THE REMAINING RISK (beyond the calendar output the shadow checks):** the incremental-PRIMARY path produces `source_miss_counts` only for the *changed* bills it actually runs through `_append_event` (the subset-STM) — reconstructing unchanged bills from cache does NOT reproduce their per-bill telemetry. So `meeting_unsourced` (the **Y2 Section-9-regression breaker** input) and `scheduleclass_*` would differ full-vs-incremental, even when the calendar is identical. The event-only shadow/replay/oracle do NOT catch this. **Required before PRIMARY drives Sheet1:** either (a) cache each bill's telemetry contribution alongside its events and re-add it for reused bills, or (b) recompute the breaker-feeding counters from the merged final event set (not per-append). Recommend a `STM_INCREMENTAL_PRIMARY=shadow` mode first (full STM still drives Sheet1+telemetry; incremental computed observe-only incl. a telemetry diff) before `=1`. Owner-facing decision; not shipped autonomously 2026-06-17.
