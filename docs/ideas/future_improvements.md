...
## ⏱️ Worker speed audit — get under ~15 min (queued 2026-06-15, prerequisite for in-session 15-min cadence)
- [x] **Step 1 — phase instrumentation (DONE, #139):** `⏱️ PHASE` marks via `time.perf_counter()` at the 5 heavy stages. Caveat: GitHub's buffered stdout collapses the lines, so the profile was hard to read from the log alone.
- [x] **Step 2 — BOTTLENECK FOUND + first big win shipped (#agenda-cache, 2026-06-15):** the real dominant cost was **NOT** the Sheet1 write (the code-level hypothesis below) — it was **re-parsing every committee agenda PDF EVERY cycle, uncached.** Owner's screenshot showed the worker step at 11m40s still running, the log flooded with `pdfminer` `Could not get FontBBox` warnings. Off-season the worker re-parsed the ENTIRE immutable adjourned-session agenda set every run. **Fix:** `Agenda_Cache` tab (`url -> bills`) reused for settled (>2-day-old) meetings — immutable dockets skip the fetch+parse entirely; recent/future always re-parse; cache gated on a `fetch_ok` success signal so a transient-empty is never frozen (see [[failures/assumptions_audit]] #89). Pure speedup, fail-safe (any cache failure → parse fresh). Pruned to 540d on load (bounded; registered in the sustainability harness). Also silenced the `pdfminer` FontBBox log flood. **Expect the off-season run to drop sharply once the first post-merge cycle populates the cache.**
- [x] **Step 3 — re-measured (warm profile, #140 + #145).** Warm 8m ≈ **245s fetches + ~181s recompute** (the 60k-row STM `iterrows` loop) + ~35s Sheets writes. **The original Sheet1-clear()+rewrite hypothesis was WRONG** — the Sheet1 write is only **12-23s** (noisy), NOT the bottleneck. The STM recompute over all of HISTORY is the inherent cost. **Stage 1 shipped (#145):** prefetch DOCKET+HISTORY blobs concurrently (`ThreadPoolExecutor`, thread-safe `safe_fetch_csv`) so the 4.7 MB HISTORY download overlaps phase 1 → verified **0.0s** download phase. BUT the ~20-40s win is **inside the ±30-60s single-run noise floor** (Sheets-API jitter: persist 23↔38s, Sheet1 12↔23s run-to-run). Lesson: trimming a fixed 8m can't make cycles *reliably* fast — only skipping the heavy work can.
- [ ] **Step 4 — STAGE 2: verified-unchanged short-circuit (the big lever, ~8m → ~20-30s off-season).** An EARLY-EXIT probe: cheaply detect whether ANY input that affects Sheet1 changed since the last successful cycle; if not, skip the entire recompute (STM + classification + persists + Sheet1 rewrite) and just refresh `AA1` + state cells. Design:
  - **Signature = content-hashes of every output-affecting input** + a `LOGIC_VERSION` constant: `HISTORY.CSV`, `DOCKET.CSV`, `VOTE.CSV` (RefidClass feeds Sheet1), the Schedule API (hash a **canonical projection** — sorted `(date,committee,time,status,agenda_url)` tuples, NOT raw JSON which may reorder), `ACTIVE_SESSION`, the committee map. Store the signature in a state cell (next free, e.g. `AB1`) + a `skip_count` cell.
  - **Cheap detection (the enabler):** Azure blobs return `ETag`/`Last-Modified` (verified 2026-06-16). `ETag` match → content definitely unchanged → no download (fast path). `ETag` differs → off-season DOCKET+VOTE are **regenerated daily** (fresh ETag at 16:00 GMT) with possibly-identical content, so download + **content-hash** to decide — NEVER trust ETag-change as content-change. HISTORY's ETag is stable through the day → most cycles skip its 4.7 MB download.
  - **[x] ETag conditional-fetch byte-reuse SHIPPED (2026-06-17, LIS-safety guardrail #1):** `safe_fetch_csv` now sends `If-None-Match` and reuses cached bytes (`.lis_blob_cache/`, persisted via the Actions cache) on a 304 — the 4.7 MB HISTORY transfer is skipped when unchanged. This is the **byte-reuse half** (always full-processes the real bytes → zero accuracy risk). The **skip-recompute half** (use the ETag/signature to skip the STM+Sheet1 rewrite entirely) is the rest of Step 4, still pending — it carries the signature-completeness risk and needs the THREE backstops below. VOTE.CSV still re-downloads (separate ragged path). See [[knowledge/lis_api_safety]].
  - **Skip iff:** `signature == stored` AND Sheet1 is intact (non-empty, expected row count) AND `skip_count < MAX_SKIP_CYCLES`. Else → today's full path, then store the new signature + reset `skip_count`.
  - **THREE backstops (so it can NEVER silently freeze stale data — Standard #2):** (1) forced full recompute every `MAX_SKIP_CYCLES` (~8 = ~24h at 3h cadence) — self-heals even if an input was missed from the signature; (2) the chained accuracy sentinel's in-session staleness gate (S1=ACTIVE + stale → CRITICAL); (3) `AA1` freshness marker keeps updating on a skip (the data IS current — inputs verified unchanged), so staleness is always visible to the site.
  - **Must hash a CANONICAL projection, not raw bytes/JSON, for any source with volatile ordering/metadata** (the Schedule API especially) or every cycle false-detects "changed". Blobs are files → raw content-hash is fine.
  - **Fail-safe = recompute:** any probe error (fetch fails, hash can't compute, Sheet1 read fails) → full recompute, never skip. The short-circuit is a pure speedup; a miss costs time, never accuracy.
- [ ] **Step 5 — session-aware cadence:** with Stage 2 making cycles cheap, run 15-30 min in-session / hourly off-season without pile-up. Pairs with the chained sentinel so detection is immediate regardless of cadence.
  - **OWNER DECISION (2026-06-17): DEFER.** Owner raised the right objection to naive "GA adjourned → run 1×/day" gating: *interim committee meetings and other off-season events still appear on the calendar between sessions*, so cadence can't key purely off the S1 ADJOURNED flag — there may be real meetings/activity to capture. The gate must account for "is there anything on the forward calendar in the next N days" (ties into the Forward-calendar block idea below), not just session lifecycle. Revisit after the incremental flip ships. The S1 flag remains a necessary input but is **not sufficient** on its own.
- [ ] **Step 6 — INCREMENTAL STM (the real IN-SEASON lever; Stage 2 only helps off-season/quiet).** Gemini (2026-06-16) correctly flagged that the short-circuit (Step 4) almost never fires in-season (HISTORY changes constantly), and the active-session bottleneck is the full 60k-row STM re-run on every cycle a new action lands. **Decisive feasibility finding (code-read, 2026-06-16):** the STM has **NO cross-bill state** — the only cross-row state is `bill_locations[bill_num]` + `last_seen_date[bill_num]` (both bill-keyed); `docket_memory`/`convene_times`/schedule map are read-only inputs in the loop. So it's **per-bill decomposable** → reprocess only the bills whose history changed (reuse `_hash_history_rows_for_bill`, already used for LegEvent hydration), carry forward the rest.
  - **MEASURED in-season delta (real 2026 HISTORY, 65,367 rows / 3,645 bills):** typical active day **324 bills changed (8.9%)**, busiest (Mar 10) **1,236 (33.9%)**, crossover ~700-920/day (~20-25%). Per-CYCLE (estimated, spread over ~8 active hrs — HISTORY is date-only, no timestamps): **~120 bills/3h cycle (~30× fewer than 3,645)**, **~10/15-min cycle (~99%+)**. So Gemini's "only 12 bills" is right only at fine cadence; per-DAY it's hundreds. Incremental STM **+ faster cadence are synergistic**.
  - **Honest scope:** incremental STM saves the ~180s STM → ~6s (8min → ~5min in-season). **Phase 1 (245s fetches + the 3,310-row schedule loop) then becomes the new wall** — incremental STM is necessary, not sufficient, for sub-minute cycles.
  - **THE hard subtlety (why the differential oracle is mandatory):** it's NOT just HISTORY-changed bills. When the schedule/docket changes (constantly in-season), bills whose *resolved TIME* changes must ALSO reprocess even though their own history didn't move. So the incremental key per bill = hash(bill HISTORY rows) **+** the shared-input entries that bill depends on. A naive "only HISTORY-changed bills" engine silently drops time updates.
  - **BUILD ORDER (replay-sim-first = step 1 of the incremental STM, NOT an alternative):** (1) extract the STM into a replayable pure-ish function; (2) build the **differential-replay harness** — feed HISTORY day-by-day, run full STM at each step (the in-season measurement + the correctness oracle); (3) build the incremental engine; (4) at each replay step assert `incremental_output == full_output` across the WHOLE 2026 session; ship only when identical. The replay harness is the safety net that makes the most-dangerous-change-in-the-system buildable without silent corruption.
  - **STATUS 2026-06-17: the flip is MERGED (#157, flag-gated, dormant) — default off, NO production change on merge.** Validation: order-invariance ✅, cache shadow ✅ (3645 reused / 0 recomputed / match), day-by-day replay ✅ (28/28). `STM_INCREMENTAL_PRIMARY` env: `""`=full STM (default), `shadow`=full drives Sheet1 + incremental runs in isolation and proves events **and** breaker telemetry identical, `1`=incremental drives Sheet1. Reuse gated on cache-loaded AND the shared-input signature unchanged (`_compute_stm_shared_sig`, ONE shared definition). Primary mode is FAIL-SAFE (any incremental error → discard partial + full STM). ~9 review rounds caught real bugs: shadow-restore-in-try (→ try/finally, else corrupt Sheet1), PHANTOM events for a bill that left HISTORY (→ skip cached bills absent from `legevent_history_hashes`), `_floor_miss_dates` leak, corrupt-cache iteration guards. TYPE FIDELITY: `_reconstruct_stm_event` restores `AgendaOrder`'s int (cache str-coerces it; the shadow can't see type — locked by `test_incremental_flip.py`).
    - **Telemetry/breaker risk RESOLVED by construction:** reconstruct unchanged bills **through `_append_event`**, so `meeting_unsourced` / `rows_appended` / `invariant_violations` (the breaker inputs) are reproduced exactly — they are pure functions of the (identical) event set; `_unconfirmed` is computed from the final set. The earlier worry (counts only for changed bills) only applied to a *direct-append* reconstruction; routing reused events back through `_append_event` closes it. The `shadow` mode's telemetry diff is the proof.
    - **SUBTLE CATCH (`_reconstruct_stm_event`):** `_stm_event_key` `str()`-coerces every field, so the cache stores `AgendaOrder` (the ONLY int field, range −100..999) as a STRING. A naive `dict(zip(...))` reconstruction would write `"-1"` where a full run writes `-1` → a mixed-type Sheet1 column — and **the shadow can't catch it** (it str-coerces both sides). Fixed by restoring int type in `_reconstruct_stm_event` + locked by `tools/verification/test_incremental_flip.py`; `_STM_EVENT_INT_FIELDS` guards against a future non-string field being added silently.
    - **TELEMETRY PARITY (the deep finding, Gemini #157 high + owner decision 2026-06-17):** the STM moves ~25 counters; reconstruction-via-`_append_event` reproduces the breaker counters (exact) but NOT the STM-loop counters. Those split: **event-derived** (route/class/origin/floor — a 1:1 function of the cached event's `Origin`/`LegEventRoute`/`RefidClass`/`ScheduleClass`, so *reproducible*) and **per-row PROCESS** (`total_processed`, `dropped_noise`, `legevent_cache_hits/misses`, `legislation_event_attempted`, `*_recovered` — about rows/network the incremental SKIPPED, so **inherently irreproducible** without re-processing). **CALENDAR + breaker are 100% exact regardless.** Owner chose (2026-06-17) the **empirical** path: the shadow now logs the FULL per-counter delta on real cycles, so the event-derived reproduction gets built against ground truth (not blind-replicated), and the process-counter under-report is documented as inherent. Build order: shadow → read real deltas → reproduce event-derived counters in the reconstruction loop → shadow confirms only the (inherent) process counters differ → `=1`.
    - **Non-breaker telemetry — settled (NOT a reproduction TODO):** the STM's process counters (`dropped_noise`, `total_processed`, `legevent_cache_*`, `*_attempted`/`*_recovered`, `sourced_*`, `_floor_hit`/`_floor_miss`) describe work DONE, are NOT present in the final events, and feed nothing downstream (`_floor_hit/_floor_miss` are STM-internal; verified). In `=1` they correctly report only the changed bills processed. A partial event-derived mirror would be WRONG, not more complete, so it's deliberately not done. The shadow logs the full deltas observe-only (`all-telemetry-Δ`) so they're visible during the window. Full process parity would need per-bill delta caching — unnecessary for correctness.
    - **ROLLOUT (do NOT skip):** set repo var `STM_INCREMENTAL_PRIMARY=shadow`, confirm `✅ INCREMENTAL-PRIMARY SHADOW MATCH` (calendar + breaker) and zero `🚨 DIVERGENCE` over a window of real cycles **including a crossover-style busy day**, THEN set `=1`. Revert instantly by unsetting. CRITICAL alert (`dedup_key=stm_incremental_primary_divergence`) on any calendar/breaker divergence.
...
**Tracking:** `sustainability_audit` keeps `API_Cache` as an `unrecognised-tab` WARN until a policy is declared (register in `RETENTION_DAYS` or `BOUNDED_TABS`). Cross-ref [[architecture/stress_test_failure_modes]] Y5 + standing-open #1.

**Status 2026-06-14:** `C7_1a_RawCorpus` (+ siblings) **DONE** — archived to "Mastermind Archive" and removed from the live workbook ([[architecture/session_archive]]); cap 79.7%→75.3%. **`API_Cache` is PARKED by owner decision** — owner has their own ideas and wants a bigger conversation before any change; do NOT prune it unilaterally. It's regenerable scratch (not trend data), so it does NOT go in the session archive; the likely fix when revisited is a session-scoped prune (mirror the LegEvent cache S1 self-prune). The harness watches the cap, so there's runway.

## Session-archive auto-rollover hook (flagged 2026-06-14, post-#131; not urgent — fires at 2027 session open)
- [ ] Wire `tools/session_archive/archive.py snapshot-session` into the worker so a COMPLETED session's `Sheet1` is archived AUTOMATICALLY when the worker detects the active session changed from the last-archived one. The worker already detects rollover for the cache prune (`active_session` vs persisted) — same trigger; record the last-archived code in a state cell so it's idempotent. This is the "automatic" half of the owner's requirement; the manual `snapshot-session` mechanism already exists and was used for the 2026 first archive. The cross-session **trend-analytics** feature (per-committee survival rates) READS the archive and is a separate, later project — out of scope here.
