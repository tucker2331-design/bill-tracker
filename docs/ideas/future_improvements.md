...
- [ ] **Step 3 — re-measure after the cache lands**, then decide if the Sheet1 write needs work too. **Original code-level hypothesis (still open, now lower priority):** the **full Sheet1 clear()+rewrite** every cycle (~37.5k rows × 14 cols ≈ 525k cells, chunked, network-bound). If still heavy after the agenda cache, make it **incremental** — diff new final_df vs the live sheet, batch-update only CHANGED rows. Risk: clear()+rewrite is idempotent/safe; an incremental diff is more complex and must be correctness-tested. The LIS hydration is already incremental (`_hash_history_rows_for_bill`).
- [ ] **Step 4 — session-aware cadence:** 15-min in-session via the schedule + a cheap-exit when HISTORY hasn't mutated; 3h off-season. Pairs with the chained sentinel so detection is immediate each cycle regardless of cadence.
