...
|---|-----------|---------------|---------------------|
| 1 | **Conditional fetch** | Never re-download unchanged data. Use ETag/`If-None-Match` / `Last-Modified` (→ 304) for blobs; content-hash to confirm; skip the download when unchanged. | ⚠️ **Partial.** Agenda PDFs cached (settled dockets skip fetch+parse); LegEvent hydration is incremental; STM events cached. **But the HISTORY/DOCKET/VOTE blobs are still re-downloaded every cycle** — the ETag layer (Stage 2, [[ideas/future_improvements]] Step 4) is designed, not shipped. **This is the #1 thing to ship before raising cadence.** |
| 2 | **Jitter** | Never hit exactly :00/:15/:30/:45 forever — randomize within the window so we don't look like a bot and aren't trivially rate-limited. | ❌ **Not present.** Cron is fixed (`0 */3 * * *`). Add ±N-min jitter when cadence rises. |
...
| 4 | **Hard ceiling** | An absolute per-day request cap as a runaway guard, independent of the cadence logic — a bug can never spike us into a ban. | ❌ **Not present.** Add a per-source daily request counter + cap (CRITICAL alert + halt if exceeded). |
