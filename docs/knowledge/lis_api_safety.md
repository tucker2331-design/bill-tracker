...
| **bill_tracker** (`🗂️ Bill Tracker`) | `40 * * * *` (hourly offer) → self-throttle: active ~1h / EMPTY ~6h; + jitter + quiet hours | 1✅ 2✅ 3✅ 4✅ 5🟡 | Added 2026-06-22; cadence made activity-correlated **PR #198**. **Inherits 1/3/4** by reusing `get_armored_session()` + `safe_fetch_csv`. Adds **#2 jitter + quiet hours** via `_scheduled_gate()` (mirrors the worker), now also **#5**: `_cadence_should_run()` reads the SHARED `Sheet1!AC1` window signal + its OWN last-run marker `Bill_Tracker!U1` — hourly when the GA is moving bills, ~6h when the forward calendar is empty. LIGHT job (~5 reads, mostly 304s). Shares the `calendar-worker` concurrency lock → never overlaps a calendar cycle's LIS use. |
