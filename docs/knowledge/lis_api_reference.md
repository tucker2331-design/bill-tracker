...
> **🧭 CLASSIFICATION FIELDS — LIS classifies every action; consume this, do NOT build a dictionary or text-match.** (Promoted to a callout 2026-05-31 after we reached for text/dictionary 4× while these sat here unused since PR-C3 — see [[failures/assumptions_audit#57]].) Each event carries LIS's OWN structural type:
> - **`ReferenceType`** — measured values (37-bill / 1,106-event probe): `Vote`, `Communication`, `LegislationText`, `Reference`, `TextVersion`.
> - **`VoteTally`** — present ⟺ a recorded vote (392/392 such events are timed meetings). The single cleanest meeting signal.
> - **`IsPassed`** (bool), **`EventDate`** wall-clock time presence.
>
> Measured routing truth: `VoteTally` present → **meeting**; `ReferenceType in {LegislationText, TextVersion}` (0 real-time) → **admin/document**; `Communication`/`Reference` are MIXED (hold timestamped admin like prefiled/referred) → need the full multi-field rule. The 942 X-Ray false positives are all non-`Vote`: H5601/S5601=`LegislationText`, G7210=`Communication`. The substring text classifier is wrong BOTH ways (142 false positives + 249 missed votes). **`Description` is the plain-English meaning to DISPLAY (identical to LIS's site); the structural fields above are for internal calendar-vs-ledger ROUTING. Never show a raw code to a lobbyist; never derive what LIS already labels.**
