...
### 41. `[H|S] (House|Senate) (sub)?committee offered` is administrative, not meeting (supersedes #37's call)
- **What broke:** After PR#18 shifted classification hygiene and PR#19 pinned the investigation window, 9 Section 9 bugs remained in crossover week: 8 rows matching `H House (sub)committee offered` / `S Senate (sub)committee offered` plus 1 Memory Anchor `recommends continuing` row. #37 had classified these as legitimate meeting actions based on phrase frequency ("not prefiled, must be meeting"). PR#22 investigation with the real refid + co-occurrence pattern proved the call was wrong.
- **How it was caught:** Direct HISTORY.CSV + LIS Schedule API + Sheet1 triangulation (no worker run needed — all three sources are public/cacheable).
  - Pulled Sheet1 via gviz, pulled HISTORY.CSV via Azure blob `lis.blob.core.windows.net/lisfiles/20261/HISTORY.CSV`, pulled LIS schedule via `/Schedule/api/getschedulelistasync?sessionCode=261`.
  - Isolated the exact 9 bug rows by replaying `classify_action()` on windowed Sheet1 ledger rows. All 9 identified by Date + Bill + Outcome.
  - For each of the 8 non-Memory-Anchor bugs, looked up the referral target committee from the bill's HISTORY and checked LIS for any schedule entry on the action date. Zero of the 8 committees had ANY LIS entry (not even TBA) on their action dates.
- **Root cause (data findings):**
  - 1,522 rows in session 261 match `^[HS] (House|Senate) (sub)?committee offered$`. Refid pattern is 100% consistent: 698 end `_H8120` (subcommittee variant), 184 end `_H8122` (committee variant), 640 are plain bill-code (Senate). `H8120`/`H8122` are LIS-internal **action type** codes, not committee codes — `resolve_committee_from_refid()` cannot map them to any committee and never could.
  - Same-bill+same-date co-occurrence check: 1,215 (80%) have a companion "recommends reporting" / "reported" / "motion" vote row that carries the actual committee vote. The "offered" row is a redundant clerical twin that binds to the same committee via `bill_locations` Memory Anchor and inherits the companion's time when LIS has a concrete entry.
  - The remaining 307 (20%) are orphans: their only peers are administrative notations (`Assigned HXX sub`, `Placed on X Agenda`, `Fiscal Impact Statement`) or nothing at all. No vote row. Orphans appear when the bill's committee did not meet on that date — the action recorded is purely "text was filed into the record."
  - In the Feb 9-13 investigation window, 8 of the 9 Section 9 bugs are orphans from the 307 bucket. 1 is a separate Memory Anchor `recommends continuing` case for HB1372 (bucket B — LIS has the committee entry but with blank ScheduleTime only; investigation deferred to PR#23).
- **Why #37's call was wrong:** #37 categorized "offered" strings by phrase-frequency ("not prefiled → must be meeting"). It did not verify the refid structure, and did not check whether the phrase co-occurs with an independent vote row. The refid-level analysis PR#22 ran proves these are clerical record entries recording that substitute/amendment text was offered into the committee's files — the kind of paperwork a clerk records, not a vote that requires the committee to be in session.
- **Fix:** Added `"subcommittee offered"` and `"committee offered"` to `ADMIN_OVERRIDE_PATTERNS` in both `pages/ray2.py` and `calendar_xray.py`. Classification order in `classify_action()` checks override before meeting/admin lists, so any outcome containing these substrings returns `"administrative"` regardless of the bare `"offered"` meeting-pattern match. Worker is untouched — this is purely X-Ray classification, the rows' calendar position and time (when they have one) are unchanged.
- **Expected metric delta (simulated against live Sheet1 in window):** Section 9 bugs: 9 → 1 (HB1372 Memory Anchor only). Ledger admin count +8. Classification matrix: 16 non-ledger rows flip from meeting → admin in-window (these are the "companion" offered rows already living at real committee times — their row position doesn't change, only their classification does). Full-session flip is approximately the full 1,522 rows.
- **Scope note:** This fix does NOT address HB1372's `recommends continuing` bug. That is bucket B (LIS entry exists but with blank ScheduleTime for the Memory-Anchor-resolved committee) and requires worker-side handling of TBA-only entries or a DOCKET-based time fallback. PR#23 territory.
- **Lesson:** When classifying phrases by frequency, check the underlying STRUCTURE before concluding semantics. A phrase that never has a resolvable committee refid and always co-occurs with a separate vote row is not the vote. The refid field is a structural signal that should be checked first — it would have caught this in #37 if we'd looked. Add "check refid pattern before classifying action strings" to the investigation playbook.
