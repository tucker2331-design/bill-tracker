...
## Index
*56 entries — grep `#N` or a keyword; full entry is the `#N.` header below.*
...
- **#51** — Recovery Side-Effect Gated Behind a Threshold That Goes PERMANENTLY FALSE After the Action (own pre-push audit #11, Gemini #209 CRITICAL, 2026-07-07)
- **#52** — Word-Boundary `` Before a Digit Class Won't Match a Letter-Adjacent Number (Gemini #209 HIGH, 2026-07-07)
- **#53** — A Fade-Out Transient That Never UNMOUNTS Leaves an Invisible, Click-Blocking Ghost (CodeRabbit + browser-verify, PR #211, 2026-07-11)
- **#54** — A Per-Key Map Overwritten Unconditionally Loses Earlier Data to a Later EMPTY Entry (CodeRabbit MAJOR, PR #211, 2026-07-11)
- **#55** — A Substring Filter on a Tool's Message Wording Also Matches a SIBLING Message Containing It (CodeRabbit on #214, 2026-07-12)
...
## 51. Recovery Side-Effect Gated Behind a Threshold That Goes PERMANENTLY FALSE After the Action (own pre-push audit #11, Gemini #209 CRITICAL, 2026-07-07)
**Pattern:** `_autoshard_witness_if_full` moved a big tab VA·Live→VA·Ops when `total_cells >= threshold`, then set a one-time flag (`AD1="ops"`) so later cycles short-circuit. The flag write was best-effort (swallow-on-fail, reconciled "next cycle"). But the reconciliation was gated behind the SAME `total >= threshold` check at the top — and the move DELETED the big tab, so `total` is now BELOW the threshold. Next cycle: flag still empty → `total < threshold` → returns "live" EARLY, never reaching the "flag it" branch. The witness is gone from VA·Live but the pointer says "live", so the worker re-creates an empty witness in VA·Live → **split-brain** (real data in VA·Ops, new writes in VA·Live), and the flag is stuck-unset forever. This is a textbook instance of **pre-push audit point #11 / assumptions_audit #51**: a state-carrying side effect *required to RECOVER* must not sit inside a gate that can become permanently unsatisfiable by the very action it follows.
**Self-check:** For any "do X, then persist a flag saying X is done" where the flag write can fail: the recovery MUST key off the ACTUAL post-state (does the tab exist in VA·Ops?), not the trigger that caused X (the threshold, now false). Structure: check the destination/actual location FIRST and reconcile the flag from it; only fall through to the trigger check when the action genuinely hasn't happened. Grep EVERY enclosing gate above a recovery write, not just the nearest. Pairs with #15 (sentinel collision) — "flag empty" must not be conflated with "not yet done" when the done-state is independently observable.

## 52. Word-Boundary `\b` Before a Digit Class Won't Match a Letter-Adjacent Number (Gemini #209 HIGH, 2026-07-07)
**Pattern:** `conditionStem` normalized alert messages by replacing numbers with `#` so per-bill firings collapse to one condition — but used `/\b[0-9][0-9,.]*\b/g`. There is NO word boundary between a letter and a digit (both are `\w`), so `SB587` / `HB923` never match (the `\b` fails at the `B|5` seam) and each bill stayed its own "distinct" condition — defeating the collapse the code was written to do. The category rollup masked it (all rolled into one category anyway), so it passed a live smoke test looking fine.
**Self-check:** `\b` asserts a `\w`↔`\W` transition; a letter→digit seam is `\w`→`\w`, NOT a boundary. To scrub embedded numbers (IDs, versions, `SB587`), drop the boundaries: `/[0-9][0-9,.]*/g`. Reach for `\b` only when you truly mean "at a word edge" (whole-word matches), never as decoration around a charclass. When a normalization's purpose is *grouping*, assert on the grouping (count distinct stems in a test), not just "the screen looks calm" — a coarser rollup downstream can hide a broken normalizer.

## 53. A Fade-Out Transient That Never UNMOUNTS Leaves an Invisible, Click-Blocking Ghost (CodeRabbit + browser-verify, PR #211, 2026-07-11)
**Pattern:** the `RefreshNotice` toast animated to `opacity: 0` after a few seconds via a `.leaving` class, but the element was ALWAYS rendered (one `phase` flag toggled the class; it never returned `null`). After the fade it sat in the DOM invisibly — and it is `position: fixed; bottom: 22px`, so an invisible, still-laid-out box lingered at bottom-centre where it can **intercept clicks**. `tsc` and a quick "does it appear?" check both passed; only a DOM-presence-over-time probe (element gone at ~9 s) caught it.
**Self-check:** an animated dismissal has THREE states, not two — entering, leaving, and **gone**. After the fade duration, return `null` (fully unmount) so nothing lingers; also give a passive overlay `pointer-events: none` so it can never block interaction even mid-fade. Verify a transient's REMOVAL, not just its appearance.

## 54. A Per-Key Map Overwritten Unconditionally Loses Earlier Data to a Later EMPTY Entry (CodeRabbit MAJOR, PR #211, 2026-07-11)
**Pattern:** `_meeting_links[(date, committee)] = (agenda, meeting)` was assigned unconditionally in the schedule loop. LIS can publish SEVERAL rows for one meeting (a real row with links, then a skeleton row with none); last-write-wins, so a trailing empty-links row **clobbers** the links an earlier row found. The sibling `api_schedule_map`/`Time` code guards exactly this ("don't overwrite a concrete time with a non-concrete one").
**Self-check:** when a dict key can appear in MULTIPLE source rows (a meeting, a bill, a date), decide the merge rule EXPLICITLY — keep-first-non-empty (`prev=m.get(k,("","")); m[k]=(new_a or prev[0], new_b or prev[1])`), never accidental last-wins. If a sibling structure in the same loop already guards multi-row overwrite, that's your tell that this one must too.

## 55. A Substring Filter on a Tool's Message Wording Also Matches a SIBLING Message Containing It (CodeRabbit on #214, 2026-07-12)
Check 17's pyflakes gate matched `"undefined name"` — which is also a substring of pyflakes' F403 wording *"'from X import \*' used; unable to detect undefined names"*, so any star-import would have failed valid pushes as a phantom undefined-name. Fixed by matching the QUOTED form `undefined name '` (unique to F821) and pinning `pyflakes==3.4.0` in CI (a filter on message wording is coupled to the version that words it). **Pre-push check:** when gating on another tool's output text, grep that tool's message catalog for OTHER messages containing your needle; match the most-specific distinguishing form and pin the tool's version.
