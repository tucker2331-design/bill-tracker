...
- **RECURRENCE — PR #110 (2026-06-09), caught by Gemini as CRITICAL:** the new LIS-authorization gate called `push_system_alert(...)` then `return`ed — *before* the sheet was even opened, so the buffered alert was unconditionally lost on the exact path (2027 halt) where notification matters most. Same pattern, weeks after #39 was written. Fix: the gate writes its halt DIRECTLY to `Sheet1!X1` with its own minimal gspread auth before returning. **Meta-lesson: any `return` added upstream of the alert-flush point re-creates this bug — run the self-check on EVERY new early-return, not just breaker paths.**
...
## 41. Two Consumers of One Shared Constant Still Parse Differently
**Pattern:** A PR unifies a magic value (e.g. the `[07:00,23:00]` meeting-hours window) into ONE shared constant used by two functions — but leaves the two functions' surrounding PARSING logic divergent. The shared constant looks consistent; the inputs reaching it are not. PR #90 unified the window but `structural_router._has_meeting_time` parsed the hour via `t.split(":")[0]` while `calendar_worker._plausible_meeting_time` used fixed slices `t[0:2]`/`t[3:5]` — so a single-digit hour (`"7:30:00"`) parses in one and raises/returns-None in the other. Latent because LIS zero-pads today.
**Self-check:** When you extract a shared constant to make two call sites "agree," also confirm the code AROUND each use agrees — same parsing, same normalization, same edge-case handling. A shared constant unifies the threshold, not the pipeline that feeds it. Prefer factoring the whole predicate (parse + compare) into one shared helper when practical, not just the literal.

## 42. Load-Fail + Dirty-Flag → Clear-and-Rewrite Wipes the Whole Store
**Pattern:** A persistent cache/store is loaded at cycle start and rewritten via `clear()`+`update()` at cycle end. The persist is gated on a `dirty` flag (did anything change this cycle?). But if the LOAD failed **transiently** (open/read timeout) and returned an *empty* in-memory store while the work still produced a few new entries (`dirty=True`), the end-of-cycle rewrite clears the real tab and writes back only this cycle's handful — **destroying the entire existing store**. The `dirty` flag answers "did it change?" but NOT "do I actually hold the full prior state?" PR #140's `_persist_agenda_cache` would have wiped thousands of cached agendas on a single `get_all_values()` blip. The trap is the same sentinel-collision family as #15/#89 in [[assumptions_audit]]: an empty-from-failure was indistinguishable from an empty-because-new.
**Self-check:** For any load→mutate→**overwrite** store, ask: *"can the load fail in a way that yields a PARTIAL in-memory copy, and would the overwrite then destroy the unread remainder?"* If yes, carry an explicit `load_ok` **boolean** (not a magic value in another return slot, and not "is the dict empty") and skip the destructive rewrite when `not load_ok`. Distinguish *absent* (safe to create) from *unreadable* (must not overwrite). A clear-and-rewrite persist is only safe when you provably hold the complete current state.

## 43. delete=False Temp File Leaks on the Exception/Early-Return Path
**Pattern:** `tempfile.NamedTemporaryFile(delete=False)` hands you a path you MUST delete yourself. If the only `os.remove(path)` sits on the success path (after the parse) and the `except` block `return`s early, every failure leaks the temp file to disk — it accumulates across items in a run. PR #140's agenda PDF parse removed the file only after a clean parse; a `pdfplumber` exception jumped to `return [], True, False`, skipping cleanup.
**Self-check:** For any `delete=False` temp (or any acquired OS resource), put the cleanup in a `finally`, not on the success path — `finally` runs even when the `try`/`except` returns. Initialize the path to `None` before the `try` and guard `if path and os.path.exists(path)` so cleanup is safe whether or not creation got that far.

## 44. Writing Beyond the Allocated Grid (update_acell Won't Auto-Expand)
**Pattern:** A new Google Sheet is the default 26 columns (A–Z); `worksheet.update_acell("AA1", …)` (or any write past the allocated `col_count`/`row_count`) raises `APIError` — gspread does NOT grow the grid for a cell/range write. PR #142 added a freshness marker at `AA1` (col 27) on a 26-col Sheet1, so the write would have failed every cycle, firing a constant `API_FAILURE` WARN and never persisting the value.
**Self-check:** Before writing to a new cell/range, confirm it's inside the current grid. If it may be beyond it, widen FIRST and idempotently: `if ws.col_count < N: ws.add_cols(N - ws.col_count)` (same for rows). `clear()` empties values but never shrinks the grid, so the widen runs once. The LegEvent cache already does this for column growth (`_open_or_create`); apply the same rule to every out-of-grid write.

## 45. Extracting a Stateful Loop: loaded-minus-stored Misses Augmented-Assignment Accumulators
**Pattern:** Lifting an inline loop into a standalone function, a free-variable proof computed as `free = loaded - stored` looks airtight but **silently drops caller-owned accumulators mutated with `+=`/`-=`** — because an `AugAssign` both *loads* and *stores* the name, so it lands in `stored` and is excluded from `free`. At runtime the function treats the name as a local (any assignment in a scope makes it local for the whole scope) and `UnboundLocalError`s on the first `+=` (read-before-assign). PR #147 extracted the STM and missed `_floor_hit`/`_floor_miss` (init'd before the loop in the caller, read after it). Compile passes; it only fails at runtime.
**Self-check:** When extracting a loop/block, find inputs via **loaded-BEFORE-stored** (use-before-def), not loaded-minus-stored. Treat every bare-name `AugAssign` target that isn't plain-assigned earlier in the new function as a required input. Scalars (`int`/`str`) must be threaded **in AND returned** (they don't propagate back like mutable dicts/Counters do); the caller captures the return. And remember a clean static proof can still have a hole — keep the runtime differential/sentinel gate as the final arbiter.

## 46. Replay/Simulation Loop: Re-groupby+iterrows a GROWING Slice Every Step → O(steps × rows)
**Pattern:** A day-by-day (or cycle-by-cycle) replay that, *inside the loop*, re-runs `df[df.date <= D].groupby(key)` + `iterrows()` to recompute per-group state from scratch each step. Each step redoes all prior work → **O(steps × rows)** with the slowest pandas primitives (`iterrows` builds a Series per row), turning a validation harness into a multi-hour job. Gemini #152 flagged it on the incremental-STM replay sim. Subtlety: Gemini attributed the *whole* runtime to the hashing, but the dominant cost was actually the **N full re-runs of the expensive inner computation** (the STM itself), not the hashing — fix BOTH or you "optimize" the cheap part and stay slow.
**Self-check:** For any replay/simulation loop: (1) **pre-extract once** via column zip (`zip(df[a].astype(str), df[b], ...)`), never `iterrows` in the loop; (2) since steps are monotonic, **accumulate incrementally** (append the step's new rows to a per-key list) and **recompute only the keys that changed this step**, not the whole slice; (3) profile what *actually* dominates — if an expensive inner call runs every step, **stride-sample** it (check every Nth step; still accumulate every step so as-of-D state stays exact). Bigger per-stride deltas are a *stronger* reuse test, not a weaker one. See also [[open_anti_patterns]] #10 (deferred or-on-nullable) — `.astype(str)` up front also neutralises the `NA or ""` ambiguity in the same extraction.

## 47. `date.today()` for an ET-Domain Comparison → Off-by-One on a UTC Runner
**Pattern:** Comparing a legislative date (a Virginia/ET concept) against `datetime.date.today()`, which returns the *runner's* local date. On a GitHub Actions box (UTC), an evening-ET run is already "tomorrow" in UTC, so a `meeting_date >= today` filter drops meetings still scheduled for **today in ET**, and a "what's new today" filter reads the wrong calendar day. Silent — it only misfires for a few ET-evening hours each day, and never on a dev laptop already in ET. This is the **read-side twin of #38** (which is the write-side: ET wall-clock mislabeled as `…Z`). Same root cause (mixing UTC and ET), opposite direction.
**Examples:**
- bill_tracker PR2 (PR #161) filtered upcoming DOCKET meetings with `today = datetime.date.today()`. Gemini (HIGH) + Qodo (Bug, HIGH) both flagged it; Qodo cited the same ET/UTC class accepted in PR #138 / #109. Fix: `today = datetime.datetime.now(pytz.timezone("America/New_York")).date()` — the repo's established convention (`calendar_worker` uses `pytz.timezone('America/New_York')` throughout).

**Self-check:** Any `date.today()` / `datetime.now()` *without* an explicit tz that is then compared against domain (legislative) dates is suspect. Grep the diff for `\.today\(\)` and bare `datetime.now()` and pin each to `pytz.timezone("America/New_York")` (for the ET "what day is it" question) or `timezone.utc` (only for fields that are genuinely UTC — see #38). The rule of thumb: **"today" in this codebase means today in Richmond**, never today on the runner.

## 48. Monitor `except` That Only `print`s — Silent Failure on the Optional-Diagnostic Path
**Pattern:** A new optional/diagnostic block (a drift monitor, a shadow cross-check) wrapped in `try/except Exception` whose handler only `print(f"⚠️ … skipped")`. On a banking-grade no-silent-failure path this IS a silent failure: if the check breaks for a cycle, the worker loses the cross-check with **no categorized alert or counter** in Sheet1/Bug_Logs — exactly the rule "every exception produces a visible alert" (Standard #6). Both CodeRabbit (Major) + Qodo (Rule violation) flagged it on the refid-shape drift monitor (PR #178).
**Self-check:** Any `except` on the worker path must `push_system_alert(…, category=…, severity=…, dedup_key=…)` AND bump a `source_miss_counts[...]` counter — `print` alone is invisible off the runner. The `dedup_key` keys on the exception TYPE so a recurring break doesn't flood. Grep the diff for `except` + `print(` with no adjacent `push_system_alert`.

## 49. Conditionally-Assigned Local Referenced on a Path Where the Condition Was False → UnboundLocalError
**Pattern:** A local assigned only inside a guard (`refid_col` set inside `if not df_past.empty:`) but referenced later by a block that runs **unconditionally** (the metrics-finalize / monitor section). On the empty path the name is never bound → `UnboundLocalError`. If that reference sits inside a `try/except`, the error is *caught* — so it doesn't crash, it **silently suppresses** the block AND (once #48 is fixed) fires a spurious WARN every empty cycle. Qodo flagged it on PR #178; same class as PR #44, and the runtime-locality twin of #45.
**Self-check:** For any local consumed by an unconditional later block, confirm it's **pre-initialized before its guard** (`refid_col = None` up top; the conditional assignment overrides it when present; the consumer guards `if refid_col`). Don't rely on a surrounding `try/except` to paper over an unbound name — that converts a logic bug into a silent skip + false alert. Grep: every name read in a `finally`/metrics/monitor block must be assigned on EVERY path reaching it (incl. the empty-input path).

## 50. Optional-Segment Regex Includes the Delimiter Char → Over-Permissive Match
**Pattern:** A namespace regex with an optional "middle" segment whose charclass includes the very delimiter that's supposed to terminate it. `_IMPACT_DOC_REFID_RE = ^[HS]B\d+[A-Z0-9]*F\d+$` — the version segment `[A-Z0-9]*` allows `F`, so a double-`F` shape like `HB1000F122F999` still matches and is classified `DOCUMENT` → routed **administrative** (because DOCUMENT maps to admin on a blank route) instead of SURFACING as `UNKNOWN_REFID`. "Surface, don't guess" (Gemini #124, same family) — an unconfirmed shape must surface, not be assumed a document. Qodo (Bug, Correctness) on PR #177.
**Self-check:** When a regex has `<prefix><optional-seg><delimiter><suffix>`, EXCLUDE the delimiter from the optional segment's charclass (`[A-EG-Z0-9]*` not `[A-Z0-9]*` when `F` is the delimiter) so only the intended single trailing marker matches; add a golden test for the double-delimiter shape → UNKNOWN. RELATED — **every heuristic owes the 3-part doc** (Standard #1): its ASSUMPTION, how it BREAKS, and the RUNTIME CHECK that detects the break. For "impact docs never surface," the runtime check is `meeting_unsourced` (Section-9): an impact row surfacing as a timeless meeting would increment it → breaker/sentinel catch it. State the check in the comment, don't leave it implicit.
