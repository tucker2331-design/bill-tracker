...
## Index
*54 entries — grep `#N` or a keyword; full entry is the `#N.` header below.*
...
- **#53** — A Fade-Out Transient That Never UNMOUNTS Leaves an Invisible, Click-Blocking Ghost (CodeRabbit + browser-verify, PR #211, 2026-07-10)
- **#54** — A Per-Key Map Overwritten Unconditionally Loses Earlier Data to a Later EMPTY Entry (CodeRabbit MAJOR, PR #211, 2026-07-10)
...
## 53. A Fade-Out Transient That Never UNMOUNTS Leaves an Invisible, Click-Blocking Ghost (CodeRabbit + browser-verify, PR #211, 2026-07-10)
**Pattern:** the `RefreshNotice` toast animated to `opacity: 0` after a few seconds via a `.leaving` class, but the element was ALWAYS rendered (a single `phase` flag toggled the class; it never returned `null`). So after the fade it sat in the DOM invisibly — and it is `position: fixed; bottom: 22px`, so an invisible, still-laid-out box lingered at bottom-centre where it can **intercept clicks**. `tsc` and a quick "does it appear?" check both passed; only a DOM-presence-over-time probe (element gone at ~9 s) caught it. Bonus: CodeRabbit had separately flagged the `setState`-in-effect that re-armed the animation (fixed via `key`-remount).
**Self-check:** an animated dismissal has THREE states, not two — entering, leaving, and **gone**. After the fade duration, return `null` (fully unmount) so nothing lingers; also give a passive overlay `pointer-events: none` so it can never block interaction even mid-fade. Verify a transient's REMOVAL, not just its appearance: assert the node is absent from the DOM after `visible + fade` ms, not merely that it showed up. A "fixed"-position ghost is the worst kind — invisible AND in the click path.

## 54. A Per-Key Map Overwritten Unconditionally Loses Earlier Data to a Later EMPTY Entry (CodeRabbit MAJOR, PR #211, 2026-07-10)
**Pattern:** `_meeting_links[(date, committee)] = (agenda, meeting)` was assigned unconditionally in the schedule loop. LIS can publish SEVERAL rows for one meeting (a real row with links, then a skeleton row with none); the last one processed wins, so a trailing empty-links row **clobbers** the links an earlier row already found. The sibling `api_schedule_map`/`Time` code guards exactly this ("don't overwrite a concrete time with a non-concrete one") — the new map simply forgot the same rule. Silent: the meeting just shows no links, indistinguishable from "LIS published none."
**Self-check:** whenever you populate a dict keyed by something that can appear in MULTIPLE source rows (a meeting, a bill, a date), decide the merge rule EXPLICITLY — first-non-empty, last-wins, max, union — never default to last-write-wins by accident. Keep-first-non-empty: `prev = m.get(k, ("",""))`; `m[k] = (new_a or prev[0], new_b or prev[1])`. If a sibling structure in the same loop already guards multi-row overwrite, that is your tell that this one must too.
