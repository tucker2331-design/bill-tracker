...
updated: 2026-07-17
status: active
open_loop: Mockup v3 drawn + owner-reviewed twice (https://claude.ai/code/artifact/ef78b6ce-4d68-410d-918d-20db9ad6605c); resolved the roster row (§5a), found a THIRD truth-class (§5b), refined amber to probabilistic-only (P20b) and added the trace-every-claim audit (P20c). ✅ CHAIR PROBE DONE (2026-07-17) — `CommitteeRoleTitle`=Chair is structural; full roster/vote ingest now SCOPED in [[architecture/roster_and_votes_ingestion]]. STILL OPEN: (1) companion-bill sourcing has no confirmed endpoint; (2) the ORCA artifacts this page recommends — Nested-Object Matrix (navigation) + CTA Matrix (permissions) — are NOT produced; the CTA Matrix should precede any D1 route; (3) Prater's/Wroblewski's/Norman's books are PAID and unread (§7); (4) predicted-lean is Tier-3 in [[ideas/predictive_lane]] — CUT until the calibration harness clears (Standard #7). Hard blocker under the picture: roster/vote ingest is scoped but UNBUILT, and its votes store must not be Sheets.
---
