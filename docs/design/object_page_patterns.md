...
updated: 2026-07-17
status: active
open_loop: Mockup v3 drawn + owner-reviewed twice (https://claude.ai/code/artifact/ef78b6ce-4d68-410d-918d-20db9ad6605c); resolved the roster row (§5a), found a THIRD truth-class (§5b), refined amber to probabilistic-only (P20b) and added the trace-every-claim audit (P20c) after I walked into my own derived-claim trap. STILL OPEN: (1) **PROBE NEEDED — does the LIS roster endpoint return a CHAIR/ROLE field?** The whip board leans on it and nobody has checked; companion-bill sourcing likewise unknown. (2) The ORCA artifacts this page recommends — Nested-Object Matrix (navigation) + CTA Matrix (permissions) — are NOT produced; the CTA Matrix should precede any D1 route. (3) Prater's/Wroblewski's/Norman's books are PAID and unread (§7). (4) Predicted-lean is CUT until there is a measurement plan (Standard #7). Hard blocker under the whole picture: member/committee ROSTER ingestion does not exist (endpoints named + inventoried, data uncollected).
---
