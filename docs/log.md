...
PR3 of the forward-build backend. **Investigation first (ban-safe):** confirmed `BILLS.CSV` is a fourth bulk blob (`lisfiles/{session}/BILLS.CSV`, 3,646 rows, one fetch) carrying `Patron_id`/`Patron_name` + structural outcome booleans (`Vetoed`/`Approved`/`Carried_over`/`Failed`/`Passed`) + committee ids + `Chapter_id` + full-text doc refs. **Probed for a subject blob — none exists** (clean 404 on SUBINDEX/SUBJECT/etc.; only BILLS/HISTORY/DOCKET/VOTE) and the `LegislationSubject` MVC endpoint is per-bill (a 3,645-call ban risk). **Built:** (1) chief **PATRON** in bulk (`patron`/`patron_id`, 0 per-bill calls — powers "by patron"); (2) **structural-first `outcome`** from BILLS.CSV's own flags (Standard #3), keyword path only as fallback — **fixes the "Continued"→carried_over bug** the PR1 keyword set missed (442 bills were wrongly "in_progress"); (3) the true **STATUS-DRIFT check** — every live bill status validated against a local `_KNOWN_BILL_STATUSES` vocabulary (distinct from the calendar's action-status axis; "Continued" is a bill-feed status the calendar vocab lacks) → WARN/DATA_ANOMALY alert on drift, **zero extra LIS calls**. **SUBJECT deferred** (no bulk source; documented in [[lis_data_inventory]] §6). New trust counters: `bills_meta_rows`, `outcome_structural`/`outcome_keyword_fallback`, `patron_present`/`patron_missing`, `unknown_bill_statuses`. **Offline-validated on real data: 3645/3645, patron 3645/3645, outcome 3633 structural / 12 keyword, 0 unknown statuses; outcome dist signed 1124 / awaiting_governor 1224 / dead 811 (=725 Failed+86 Incorporated, exact) / carried_over 443 / vetoed 31 / in_progress 12.** Awaiting bot review → merge → live-validate.
