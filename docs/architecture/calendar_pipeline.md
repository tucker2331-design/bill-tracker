...
- **Honors every published clock exactly.** A concrete `ScheduleTime` always wins (even when the same name also has a relative entry that day) — the resolver never re-derives a time LIS published. Validated offline: 0 of 2,877 published-clock `(date,name)` keys resolve to a derived time (`tools/edge_case_replay/validate_relative_chains.py`).
...
- **Residual (~32/450 rows):** chains whose root is itself unresolvable stay at the `06:00`/`23:59` sentinel; the honest surfacing of these (top-of-day + highlight, §7.2) is the queued follow-up, gated on a structural `TimeClass` flag.
