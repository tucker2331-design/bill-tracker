...
LIS Schedule API -> build_time_graph() -> resolved_parent_map  {(date, name): "HH:MM"}
...
### `build_time_graph` — the `SortTime` resolver (DATE-AWARE, 2026-07-02)
`build_time_graph(schedules)` produces `resolved_parent_map`, the per-meeting `SortTime` (intra-day ordering key; the DISPLAYED `Time` stays LIS's verbatim string). LIS publishes many committee/subcommittee times **only as prose** ("½ hour after adjournment of the House", "Immediately upon adjournment of the House Appropriations Committee", "Immediately after the Transportation & Public Safety Subcommittee") — a transitive within-day dependency. The resolver:
- **Resolves each DAY independently** and keys the output `{(date_str, name): "HH:MM"}` (`_resolve_one_day`). Before 2026-07-02 it was NAME-keyed with no date, so a committee's 27 dated meetings collapsed to one value and a single "house adjourned" clock stood in for all 443 dates — the relative chains were mis-sorted (assumptions_audit #95, [[ideas/calendar_chain_ordering]] §8). The caller (the live-Schedule loop in `run_calendar_update`) looks up `(date_str, owner_lower)` with the SAME whitespace-collapsed lower-casing.
- **Honors every published clock exactly.** A concrete `ScheduleTime` always wins (even when the same name also has a relative entry that day) — the resolver never re-derives a time LIS published. Validated offline: 0 of 2,889 published-clock `(date,name)` keys resolve to a derived time (`tools/edge_case_replay/validate_relative_chains.py`).
- **Anchors a committee reference to the committee, not the floor.** `_committee_parent` matches "adjournment of the House **Appropriations Committee**" to that day's Appropriations node (so a subcommittee sorts after its parent), reserving the published "[chamber] adjourned" floor clock for BARE chamber references. A zero-offset ("immediately") child is nudged +1 min past its resolved parent so a transitive A→B→C chain stays strictly ordered. Front-end needs no change — `web/src/data/calendar.ts toMinutes()` already sorts by `SortTime`.
- **[REVERTED 2026-07-11 — §9d, not in production; regressed `meeting_unsourced`, see [[../failures/assumptions_audit#101]].]** Matches on the token MULTISET, and refuses on a tie (§9d). LIS names a subcommittee by repeating the distinctive word ("…Natural Resources **Agriculture** Subcommittee"), which a token *set* collapses into its own parent's name; a multiset keeps them apart. Where the reference is a bare ordinal ("Subcommittee #2") it is read as a **self-lineage** reference and scoped to the referring node's committee. If two candidates still tie, the resolver **refuses** and the row stays unresolved — honest-absent over plausible-wrong (Standard #3). See assumptions_audit #100/#101.
- **[REVERTED 2026-07-11 — §9 anchor ladder, not in production; see [[../failures/assumptions_audit#101]].]** Anchor ladder for IMPLICIT anchors (§9). Three strictly-additive rungs run only after every rung above returns `None`, so no already-resolved row can move: (5) sibling token-overlap, (6) `"full committee"` → the node's own parent from its `Parent - Sub` name, (7) a phrase naming **no** body at all → its own chamber's floor marker, picking adjourned-vs-recessed by the phrase's verb. Rung 7 is guarded: a phrase that names a body nobody matched must never fall through to the floor. `ANCHOR_RUNG_COUNTS` reports which rung placed each row, and `anchor_unresolved` is the drift canary.
- **Residual: 1/450 rows.** `2025-02-22 house convenes` points at *"the Recess of the 2024 Special Session I"* — a prior session with no node in that day's graph. Correctly refused. `TimeClass` marks it `relative_unresolved` and the front-end renders it as unplaceable rather than guessing.
...
The `Origin` column was added in PR-A. Enumerated values: `api_schedule`, `convene_anchor`, `legislation_event` (PR-C3), `journal_default`, `floor_miss`, `admin_default` (PR-C7.1g), `sibling_meeting` (PR-C7.1j), `scheduled_future` (PR-FC1 — registered, no producer yet), `executive_default` (PR-C8.4b — action-required governor action; surfaces under 🏛️ Governor), `derived_standing` (#76 — the flagged last-resort ASSUMED time, e.g. SJ209→5:34 PM; carries a real clock time), `system_alert`, `system_metrics`. `admin_default` is a journal_default row the structural router classified `route=="admin"` whose time recovery was deliberately skipped (no wrong document-batch timestamp); it collapses into 📋 Ledger Updates like journal_default/floor_miss but does NOT count `unsourced_journal` (it's structurally administrative, not merely unmatched). **NB (2026-07-03):** the per-row `TIMING_LAG` WARN the `journal_default` block once emitted was DROPPED — a no-schedule-match deferral to Ledger is a routine disposition, not an anomaly (Standard #8); it floods the operator feed and the only genuine anomaly (a MEETING action without a time) is tracked separately by I4/`meeting_unsourced`. Rows stay visible via the `[NO_SCHEDULE_MATCH]` marker + `diagnostic_hint` + the `unsourced_journal` counter. `sibling_meeting` is a timeless meeting-routed *secondary* split-action row (e.g. "Rereferred to Y" — LIS HISTORY splits this off from the combined "Reported from X and rereferred to Y" committee vote) that **inherited** the Time/SortTime/Committee of a same-`(Bill, Date)` resolved committee/floor meeting, applied only when that meeting time is unambiguous; it carries a real time (concrete source for I3) and does NOT collapse to Ledger. One `SYSTEM_METRICS` row per run carries a JSON-encoded snapshot of the source-miss counters (`total_processed`, `sourced_api`, `sourced_convene`, `sourced_legislation_event`, `unsourced_journal`, `unsourced_anchor`, `dropped_ephemeral`, `dropped_noise`, `floor_anchor_miss`, `legislation_event_attempted`, `legislation_event_recovered`). X-Ray Section 0 parses this row to render the denominator.
...
- **Sheet1 schema migration:** no `LegEventType` column added in PR-C7. The cache classifies bills internally but doesn't surface structural EventType to Sheet1 consumers.
  ...
  **Update (PR-C7.1b-1, merged):** Sheet1 gained an additive `LegEventRoute` column (`meeting`/`admin`/`""`) sourced from the dictionary-free structural router (`structural_router.py`, routes on LIS's own `ReferenceType` / `VoteTally` / `Status`). The router replaces the earlier `LegEventType` aspiration — no per-code dictionary to maintain. Observability-only at C7.1b-1 (does not change `Origin`, `Committee`, row placement, or `meeting_unsourced`).
- **X-Ray classifier rewrite:** `pages/ray2.py` `classify_action()` originally used only `MEETING_ACTION_PATTERNS` substring matching. This was the source of the ~942 false-positive "meeting bugs" measured in PR-C7.1d (`H5601`/`S5601` "Bill text as passed", `G7210` "Governor's Recommendation").

  **Resolved (PR-C7.1b-2, this PR):** `classify_action(outcome_text, legevent_route="")` now reads `LegEventRoute` FIRST (route wins when present) and falls back to text patterns only when the route is blank/unknown. Default arg keeps any external caller working; unseen future route values (LIS introducing a new category) fall through to text — never silently mis-routed. Section 9 displays a self-proving counter: "before" = text-only flagged subset, "after" = route distribution showing the admin-recovery split. Diff-identical mirror to `calendar_xray.py`.
