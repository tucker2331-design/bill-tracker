...
| S3 | **Stale investigation window** — config pinned to 2026. | LOW | **FIXED (post-C8) — claim corrected 2026-06-14.** The scrape/processing window is now FULLY auto-derived from the Session API each run (the `get_active_session_info` → `extract_dates` path); `INVESTIGATION_*` survives ONLY as the offline last-resort fallback. The old manual annual bump **and its #1r staleness WARN were REMOVED** — there is no annual transition touchpoint anymore. Asserted continuously by `sustainability_audit` TEMPORAL (`window-auto-derived`). *(This row and [[verification_durability]] both described the obsolete #1r WARN — fixed.)* |
...
| # | Failure mode | Sev | Status |
|---|--------------|-----|--------|
| Y1 | **Hardcoded API keys** (`API_KEY`, `LIS_PUBLIC_API_KEY`) — if LIS rotates them, total break with a 401. | LOW | **HARDENED (#135):** a 401/403 on the session-API gateway now raises a DISTINCT CRITICAL (`lis_api_auth_failure` — "key rotated; update API_KEY/LIS_PUBLIC_API_KEY") instead of hiding in the every-cycle OFFLINE noise. Keys are public (SPA bundle) so historically stable; rotation is now loud + actionable. |
...
| Y4 | **Dependency rot** — runtime/action/library churn. | LOW | **Node-20 CLOSED (#135):** all workflows bumped to `checkout@v4`/`setup-python@v5` (Node 24) ahead of the 2026-09-16 deadline. Remaining: periodic gspread/pandas major bumps + Python EOL (3.11 ~2027) — routine, no near deadline. |
| Y5 | **Witness / API_Cache unbounded growth** — append-only across many sessions. | **ACTIVE** | **Witness: FIXED (#126)** — the L3b nightly prune (`tools/witness_retention/prune.py`) deletes `Schedule_Witness` rows >90d, sharing the worker's concurrency group for exclusive tab access. *(It did NOT exist before #126, despite the prior "retention prune exists" claim here — the tab is 51d old so no violation had occurred yet; the prune is preventive.)* **API_Cache + workbook capacity: FIXED (#131 + #134).** `sustainability_audit` CAPACITY caught the workbook at 79.7% of the cap. Resolution: `C7_1a_*` archived to the separate archive workbook (#131); `API_Cache` was 353k ALLOCATED rows but only ~1,632 DATA rows (a stale grid, not unbounded data) → trimmed to 10k (#134). **Workbook 79.7% → 54.8%.** New harness `wasted-grid` check (allocated ≫ populated) catches this class going forward + an `archive-cells` check watches the archive's own cap. See [[architecture/session_archive]]. |

## The standing OPEN items (prioritized) — slimmed 2026-06-15

The actionable code items are CLOSED. What remains is inherent (external) or a process choice:

1. **S7 — bot reviewer replacement** (process, deadline **2026-07-17** — the nearest real one). Gemini Code Assist sunsets; pick CodeRabbit/Qodo or rely on Codex. Owner decision.
