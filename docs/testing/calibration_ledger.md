...
**Machine-readable source of truth: `calibration_ledger.json`. This page is its rendering.**

**Before proposing any stat, grep this table.** If it is here, it has been tested and the result stands
unless the METHOD changes — new sessions, a corrected definition, or a fixed leak.
...
CONDITIONAL (helps one outcome, harms the other) · DISQUALIFIED (invalid measurement — the score is
meaningless, not merely low).
...
| `text_versions` | 32.8% | 77.9% | **DISQUALIFIED** | LEAKAGE — a bill acquires text versions BY advancing (substitute/engrossed/enrolled). 1 version passed 3%, 4 versions passed 100%. Scores are meaningless. |
| `patron_floor_win_rate` | 5.5% | 9.0% | **DISQUALIFIED** | CONTAMINATED — used same-session votes. Prior-session: -5.6%. |
| `patron_party_loyalty` | 6.5% | 6.8% | **DISQUALIFIED** | CONTAMINATED — used same-session votes. Prior-session: -2.0%. Do not re-test as-is. |
| `patron_agrees_with_cmte` | 6.0% | 5.0% | **DISQUALIFIED** | CONTAMINATED — used same-session votes. Prior-session: -23.3% (committee rosters change between sessions). |

## Not yet tested — no result exists, do not assume one

Timing within the session · docket position (no pre-2025 docket data) · vote margins · companion bills ·
...
2. A definition is corrected — a DISQUALIFIED stat may be re-tested only with the leak removed, and the
   old score must not be quoted.
3. The outcome definition changes.

Anything else is a re-run of work already done. See [[testing/calibration_results]] for the narrative and
[[testing/calibration_scope]] for the method's known weaknesses.
