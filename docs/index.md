...
- [[ideas/meeting_agenda_links]] — **PLAN (measured, not built):** surface the agenda-PDF + livestream links LIS buries in the Schedule `Description` HTML. Needs a Sheet1 migration (29→31 cols); the current `agenda_url` points at a non-agenda page 89×.
- [[ideas/auto_refresh_on_new_data]] — **PLAN (probed, not built):** kill the manual-reload — the SPA loads once and never polls. Freshness-gated background refresh (poll ~40 B cells, full-fetch only on a timestamp advance). Streamlit's old auto-refresh pain no longer applies (product is a SPA).
