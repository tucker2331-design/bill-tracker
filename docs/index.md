...
- [[testing/member_signals]] — **INDEPENDENT VOTERS, BLOCS, PIVOTALITY: one collapse and two nulls. Only 21% of committee roll calls have a caucus that splits — every member-level stat must use that denominator. Unpredictability is a real trait (split-half r=0.71) but collapses into plain defection rate (r=0.92) and SUBJECT ADDS NOTHING. Blocs are a NULL once chance agreement is computed from FOLLOW rates instead of yes-rates: 132 significant pairs become 1. Pivotality is a NULL — observed 3.7% against an 11.3% party-line null. Ship the raw contested-vote follow count; nothing more survives.**
