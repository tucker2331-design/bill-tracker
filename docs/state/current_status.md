...
[[knowledge/history_refid_namespace]]. **C8.1 + C8.1b MERGED (PRs #111, #112, shadow) — STRUCTURAL EVIDENCE LAYER COMPLETE:** RefidClass batch-notice law solves the journal_default 16% at 98.3%, 0 counterexamples, K=2; next C8.1b = ScheduleTypeID carry (API_Cache schema migration → api_schedule_map → ScheduleClass stamp) for the ~964 api_schedule-text rows; plan in [[architecture/pr_c8_structural_classification]] §C8.1b. **NEXT: C8.2 — the FLIP: classify_action consumes RefidClass + ScheduleClass + LegEventRoute; DELETE the text-pattern lists from ray2/calendar_xray. Gated by the zero-unexplained-diff check (spec Hard Rule 7).** Three PRs: C8.1 shadow telemetry → C8.2 flip (text
