...
**ONE OPEN ITEM (owner action):** create a Slack Incoming Webhook (owner has an existing "Bill Tracker Bot" app in the *Voters of Tomorrow* workspace, but bill-tracking.slack.com is the owner's own workspace — recommended target) and add it as repo secret `SLACK_WEBHOOK_URL` to activate the channel. **Next worker idea (deferred):** the profile shows `setup + all fetches` still dominates even warm — investigate whether the non-agenda fetches (schedule/docket/witness) can go incremental. [[ideas/future_improvements]] worker-speed.
