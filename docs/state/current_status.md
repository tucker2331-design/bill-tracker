...
**✅ SECTION 9 = 1 MEASURED & DURABLE (2026-06-05). The single remaining row is SJ209 — genuinely irreducible (LIS published no 3/10 P&E meeting). Trajectory: 1,072 → … → 6 (first REAL cold-start measurement) → 4 (after #87) → 4 (after #74 — common clerical codes swept, but the residual was a cache bug) → 1 (after #75/PR-C7.1v cache migration). ~99.9% reduction, zero fabricated times, 0 mis-timed (safety check).** Cache 100% migrated (0 `"?"`); crossover `meeting_in_ledger` holds 9→0.
...
- **X-Ray Section 9 — route-aware (current production):** **1** ✓ (down from 1,072 peak — a **99.9% reduction**). MEASURED via a faithful reproduction of ray2.py `bug_count = len(mt_without)` against the live Sheet, with the production raw-string read. The single row is **SJ209** — P&E voted 13-Y on 3/10 but LIS published no 3/10 P&E meeting (time exists in no source). Cache fully migrated (0 `"?"`), so the count is durable. Safety: 0 committee reports mis-timed to convene.
