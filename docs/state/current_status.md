...
- **2026-08-05 — SUBJECT COVERAGE 65% -> 80%, and 2023 was MISSING from every patron finding ([[testing/subject_labels]]).** Owner asked for +10-15 points; accuracy was already 97.8%, so the number with headroom was coverage. **Union 80%** (18,234/22,659), both spaces still clearing 95%; per-session rose everywhere (2017 53%->65%) and the labelled-vs-unlabelled composition gap FELL to 4-5%. **The blocker was our own gate:** `abstract_eval` trained on 75% of ONE session while production trains on four, so it read 95.5% where the held-out cold session read 97.3% — and being the binding constraint, its pessimism was spent as lost coverage. **Found on the way, and bigger: chief patrons in 2023 vs 2024 had ZERO overlap of 141 names** (2023 writes `Adams, D.M.` / bare `McPike`), so 2023 resolved a party for **3%** of its bills against 95-99% elsewhere — ~2,000 bills sat outside EVERY majority/minority finding, computed on 17 sessions and described as 18. Now 91%; corpus-wide 88%->97%; new `chief_key` canonical patron identity (466->310 distinct patrons; seniority had been resetting each format change). Audits [[failures/assumptions_audit|#115]] + **#116**.
