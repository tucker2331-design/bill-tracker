...
## Active focus
**PR-C2 open — gap detection + witness log + reconciliation (+ round-2 Gemini patches pushed).** Second PR in the PR-C series; closes the loop from PR-C1 scaffolding. Three-part scope: (A) Y1 gap detection at cycle top, classifying `gap_cause` (first_run / future / stale / malformed / breaker_carryforward / outage / normal) and emitting WARN @ >20 min, CRITICAL @ >60 min; (B) `Schedule_Witness` append-only change-feed tab capturing ADDED + CHANGED LIS Schedule API deltas (whitelist = `Time, SortTime, Status, Location`), auto-created on first delta, NOT gated by the circuit breaker; (C) gap-triggered HISTORY-vs-witness reconciliation (capped at 7 days) that flags "confirmed blind-window loss". Round-2 Gemini patches on the same branch: Location/Room added to delta whitelist with a one-time migration burst guard; in-cycle 90-day prune REMOVED (`append_rows` + `delete_rows` same-cycle race on the witness tab) and replaced with a row-count canary — retention deferred to an L3b Nightly Audit ([[ideas/future_improvements#L3b Nightly Audit — Schedule_Witness retention owner (flagged 2026-04-24, PR-C2 round-2)]]). PR-C2.1 will follow with a Playwright historical scraper using `wait_for_selector()` (not networkidle) + ≥15s timeouts ([[ideas/future_improvements#PR-C2.1 — Playwright historical scraper (deferred from PR-C2)]]). Zero bug-count delta from C2 alone — observability + data-recovery infrastructure. CRITICAL alerts (`y1_stale`, `gap_reconciliation_oversized`, `gap_critical`) are flagged for future re-routing to a dedicated dashboard; see [[ideas/future_improvements#Notification Routing (flagged 2026-04-24, PR-C2)]]. See [[architecture/calendar_pipeline#Gap Detection + Witness Log + Reconciliation (PR-C2)]].
