...
5. **Incident counter — UNBLOCKED (owner 2026-07-17: "we need to prove it — remember the counter"), now a READY build task.** Definition = the owner's sentence ("how long data holds clean before intervention", manual interventions count); display = Health-first; guards = sentinel/tripwire/reconciliation. Wiring plan in [[architecture/incident_counter]] — first make the write-path verifiable via a scratch-workbook env override (audit #74), **seed genesis EARLY (every un-seeded day is thrown-away provable trust)**. Next engineering PR.
