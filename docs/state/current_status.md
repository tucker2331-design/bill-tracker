...
**⏱️ WORKER SPEED AUDIT — IN PROGRESS (#140 OPEN, 2026-06-15).** Owner flagged the ~20-min worker run ("audit of the 19m worker looking for ways we can speed it up"). #139 added `⏱️ PHASE` timing; owner's screenshot then revealed the true bottleneck — **re-parsing every committee agenda PDF every cycle, uncached** (log flooded with `pdfminer` FontBBox warnings), NOT the Sheet1 write the hypothesis had guessed. **#140 (awaiting Gemini):** `Agenda_Cache` serves settled (>2d-old, immutable) agendas from cache and skips the fetch+PDF-parse; recent/future always re-parse. Pure speedup, fail-safe, `fetch_ok`-gated so transients aren't frozen ([[failures/assumptions_audit]] #89), 540d-bounded. Expect a sharp off-season drop once the first post-merge cycle populates the cache. **Next after merge:** re-measure (does Sheet1 write still need incremental?), then session-aware cadence. Full plan: [[ideas/future_improvements]] worker-speed.
