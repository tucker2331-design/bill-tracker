...
**⏱️ WORKER SPEED AUDIT — #140 MERGED (2026-06-15), AWAITING LIVE RE-MEASURE.** Owner flagged the ~20-min worker run ("audit of the 19m worker looking for ways we can speed it up"). #139 added `⏱️ PHASE` timing; owner's screenshot then revealed the true bottleneck — **re-parsing every committee agenda PDF every cycle, uncached** (log flooded with `pdfminer` FontBBox warnings), NOT the Sheet1 write the hypothesis had guessed. **#140 (merged, 3 Gemini rounds — r1 critical cache-wipe → `load_ok` boolean, r2 medium temp-leak → `finally`, r3 high state-cell = false positive on the wrong worksheet, dismissed with evidence):** `Agenda_Cache` serves settled (>2d-old, immutable) agendas from cache and skips the fetch+PDF-parse; recent/future always re-parse. Pure speedup, fail-safe, `fetch_ok`-gated so transients aren't frozen ([[failures/assumptions_audit]] #89), 540d-bounded. **Verification pending:** 1st post-merge cycle populates the cache (still slow); 2nd cycle should drop sharply — confirm the worker stays green + accuracy unchanged (chained sentinel) + the runtime delta. **Next:** re-measure (does the Sheet1 write still need incremental?), then session-aware cadence. Full plan: [[ideas/future_improvements]] worker-speed.
